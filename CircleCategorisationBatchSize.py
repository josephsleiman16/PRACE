...
batches=[4,8,16,32,64,128,len(y_train)]
for batchsize in batches:
    model2 = generate_NN(3, 128, "relu", 1, "sigmoid", "adamax")
    history = model2.fit(x_train.values, y_train.values, epochs=20, validation_split=1/41, callbacks=cb_list, verbose=0, batch_size=batchsize)
    #model2.summary()
    _,test_acc = model2.evaluate(x_test,y_test)
    print("Model with Batch Size: ", batchsize, ", has Test Accuracy: ", test_acc)
    plt.plot(history.history['acc'],label="train")
    plt.plot(history.history['val_acc'],label="val")
    plt.legend()
    plt.show()
    plt.savefig("AccuracyForBatchSize"+str(batchsize)+".png")
    plt.clf()
