# -*- coding: utf-8 -*-
"""
Created on Thu Jul 22 11:18:37 2021
...
activs=["relu", "sigmoid", "softmax", "linear", "tanh", "elu", "softplus", "selu"]
for activation in activs:               
    opt = RMSprop()
    init=glorot_normal()
    model2 = generate_NN(3, 128, activation, 1, "sigmoid", opt, init)
    history = model2.fit(x_train.values, y_train.values, epochs=20, validation_split=1/41, callbacks=cb_list, verbose=0)
    _,test_acc = model2.evaluate(x_test,y_test)
    print("Model with Hidden Activation Function : ", activation, ", has Test Accuracy: ", test_acc)
    ...
    plt.xlabel("Epochs")
    plt.legend()
    plt.savefig("AccuracyFor"+str(activation)+".png")
    plt.clf()
