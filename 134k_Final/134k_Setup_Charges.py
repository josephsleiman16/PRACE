...
#setting up charges in sorted manner and appending zeros so all charges are same length
sorted_charges = []
for structure in sorted_structures:
    temp = structure.get_initial_charges()
    if len(temp) != max_atoms:
        val = max_atoms-len(temp)
        temp = np.append(temp,np.zeros(val))    
    sorted_charges.append(temp)
...
def get_CM_fv():
    ...
    feature_vector = cm.create(sorted_structures, n_jobs=1)
...
def get_SOAP_fv():
    ...
    feature_vector = soap.create(sorted_structures, n_jobs=1)
    return feature_vector
...
def get_ACSF_fv():
    ...
    for i in range(len(feature_vector)): #molecule
        for j in range(len(feature_vector[i])): # j atoms in ith molecule
...
for i in range(len(sorted_structures)):
    if i%5 == 0:
        ...
        y_test.append(sorted_charges[i])
    else:
        x.append(feature_vector[i])
        y.append(sorted_charges[i])
